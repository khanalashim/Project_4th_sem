    ...
    if (in_array($f_ext, $allowed) && in_array($b_ext, $allowed)) {
        if ($fronterror === 0 && $backerror === 0) {
            if ($frontsize < 10000000 && $backsize < 10000000) {
                ...
                $frontdestination = 'sahaback/verifyimg/' . $frontnewname;
                $finalfrontdestination = 'verifyimg/' . $frontnewname;

                $backnewname = uniqid('', true) . '.' . $b_ext;
                $backdestination = 'sahaback/verifyimg/' . $backnewname;
                ...
                $query1 = "INSERT INTO verification (user_id,front_img,back_img) VALUES ('$user_id','$finalfrontdestination','$finalbackdestination')";
                $query = "UPDATE users SET verify_request='true' WHERE id='$user_id'";
